...
from .forms import AtividadeForm, EntregaForm, ReabrirPrazoForm
...
class AtividadeCreateView(AtividadeMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse(
            ...
            kwargs={"pk": self.kwargs.get("pk"), "atividade_pk": self.object.pk},
        )
...
class AtividadeDetailView(AtividadeMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        alunos_turma = (
            self.get_turma()
            .alunos.filter(matriculas__turma=self.get_turma(), matriculas__ativa=True)
            .distinct()
        )
...
class EntregarAtividadeView(AlunoAutenticadoMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if (
            entrega.pk
            and entrega.status in ["entregue", "atrasada", "avaliada"]
            and not self.atividade.permitir_reenvio
        ):
            messages.error(self.request, "Esta atividade não permite reenvios.")
            return redirect(
                "turmas:portal_minha_area", token=self.atividade.turma.token_publico
            )
        ...
        messages.success(
            self.request,
            f"Atividade entregue com sucesso! Status: {entrega.get_status_display()}",
        )
        return redirect(
            "turmas:portal_minha_area", token=self.atividade.turma.token_publico
        )
...
class DownloadEntregasZipView(AtividadeMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        atividade = get_object_or_404(
            Atividade, id=self.kwargs.get("atividade_pk"), turma=self.get_turma()
        )

        entregas_com_arquivo = atividade.entregas.exclude(arquivo="").select_related(
            "aluno"
        )

        if not entregas_com_arquivo.exists():
            messages.warning(
                request, "Não há arquivos de entrega para baixar nesta atividade."
            )
            return redirect(
                "turmas:atividade_detalhe",
                pk=atividade.turma_id,
...
class DownloadEntregaArquivoProfessorView(AtividadeMixin, View):
    """Download autenticado do anexo de uma entrega pelo professor."""

    def get(self, request, *args, **kwargs):
        entrega = get_object_or_404(
            Entrega.objects.select_related("atividade", "aluno"),
            pk=self.kwargs["entrega_pk"],
            atividade_id=self.kwargs["atividade_pk"],
            atividade__turma=self.get_turma(),
        )
        ...
        return FileResponse(
            ...
            as_attachment=True,
            filename=os.path.basename(entrega.arquivo.name),
        )


class DownloadMinhaEntregaArquivoView(AlunoAutenticadoMixin, View):
    """Download autenticado do próprio anexo enviado pelo aluno."""
...
class AtividadeListaPublicaView(TurmaPublicaMixin, ListView):
    ...
    def get_queryset(self):
        return Atividade.objects.filter(turma=self.turma, publicada=True).order_by(
            "prazo"
        )
...
class AtividadeDetalhePublicoView(TurmaPublicaMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        return get_object_or_404(
            Atividade, id=atividade_id, turma=self.turma, publicada=True
        )
...
class AvaliarEntregaView(ProfessorRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "avaliacoes/_inline_avaliacao.html",
            {
                "entrega": entrega,
                "form": form,
                "edit_mode": True,
...
class ReabrirPrazoAlunoView(AtividadeMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse(
            ...
            kwargs={
                "pk": self.kwargs["pk"],
                "atividade_pk": self.kwargs["atividade_pk"],
            },
        )
