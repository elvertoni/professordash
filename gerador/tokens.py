...
def estimar_custo(modelo: str, texto_entrada: str, num_aulas: int = 1) -> dict:
    ...
    tokens_input_est  = int(len(texto_entrada) / CHARS_POR_TOKEN)
    tokens_output_est = TOKENS_OUTPUT_AULA * num_aulas
    custo = calcular_custo(modelo, tokens_input_est, tokens_output_est)
    preco = PRECOS_MODELOS.get(modelo, PRECO_PADRAO)
    return {
        'modelo':            preco['nome'],
        'tokens_input_est':  tokens_input_est,
        'tokens_output_est': tokens_output_est,
        'tokens_total_est':  tokens_input_est + tokens_output_est,
